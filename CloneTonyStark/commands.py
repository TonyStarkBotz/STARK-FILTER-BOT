...
from CloneTonyStark.database.clone_bot_userdb import clonedb
from info import *
...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        filesarr = []
        for file in files:
            tony_file_id = file.file_id
            k = await temp.BOT.send_cached_media(chat_id=PUBLIC_FILE_CHANNEL, file_id=tony_file_id)
            tony = await client.get_messages(PUBLIC_FILE_CHANNEL, k.id)
            mg = getattr(tony, tony.media.value)
            file_id = mg.file_id
            files_ = await get_file_details(tony_file_id)
            files1 = files_[0]
            title = '@TonyStark_Botz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))
            size=get_size(files1.file_size)
            f_caption=files1.caption
            if f_caption is None:
                f_caption = f"@TonyStark_Botz  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))}"
            if cd["update_channel_link"] != None:
                up = cd["update_channel_link"]
                button = [[
    ...
    k = await temp.BOT.send_cached_media(chat_id=PUBLIC_FILE_CHANNEL, file_id=file_id)
    tony = await client.get_messages(PUBLIC_FILE_CHANNEL, k.id)
    m = getattr(tony, tony.media.value)
    file_id = m.file_id
    msg = await client.send_cached_media(
        chat_id=message.from_user.id,
        file_id=file_id,
...
async def settings(client, message):
    ...
    try:
        ...
        link = 'https://t.me/TonyStark_Botz'
