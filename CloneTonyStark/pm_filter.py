    ...
    btn = [
        [
            InlineKeyboardButton(
                text=f"[{get_size(filetony.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), filetony.file_name.split()))}", callback_data=f'file#{filetony.file_id}'
            ),
        ]
        for filetony in files
    ]
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn = [
        [
            InlineKeyboardButton(
                text=f"⚜️[{get_size(filetony.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), filetony.file_name.split()))}", callback_data=f'file#{filetony.file_id}'
            ),
        ]
        for filetony in files
    ] 
...
async def advantage_spell_chok(client, name, msg, reply_msg, tony_search):
    mv_id = msg.id
    mv_rqst = name
    ...
    if tony_search == True:
        tony_search_new = False
        tony_ai_msg = await reply_msg.edit_text("<b><i>Advance Ai Of Tech tony Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        movienamelist += [movie.get('title') for movie in movies]
        for techtony in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(techtony[0]):
                await auto_filter(client, techtony, msg, reply_msg, tony_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
